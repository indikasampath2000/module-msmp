...
`msmp` connector allows you to perform
...
`msmp` connector uses `JMS connector` and `HTTP connector` underneath. It has two main components. 

* `msmp:Client` is used to store messages. The messages are stored into a queue of a message broker. 
* `msmp:MessageForwardingProcessor` is used to poll messages from the store (i.e queue in message broker), and 
...
forwarding, `msmp` connector makes use of resiliency provided by HTTP Client of Ballerina. In case of a failure case,
we need to keep the message in the queue. Hence, as the acknowledgement mode the `msmp:MessageForwardingProcessor` component
...
```ballerina
    ...
    msmp:ForwardingProcessorConfiguration myProcessorConfig = {
...
```ballerina
    msmp:ForwardingProcessorConfiguration myProcessorConfig = {
        ...
        retryInterval: 3000,
        maxRedeliveryAttempts: 5,
        forwardingFailAction: msmp:DEACTIVATE
    };
```
...
```Ballerina
    msmp:MessageStoreConfiguration dlcMessageStoreConfig = {
        messageBroker: "ACTIVE_MQ",
        providerUrl: "tcp://localhost:61616",
        queueName: "myDLCStore"
    };

    msmp:Client dlcStoreClient = new msmp:Client(dlcMessageStoreConfig);
    ...
    msmp:ForwardingProcessorConfiguration myProcessorConfig = {
        ...
        retryInterval: 3000,
        maxRedeliveryAttempts: 5,
        forwardingFailAction: msmp:DLC_STORE,
...
var myMessageProcessor = new msmp:MessageForwardingProcessor(myProcessorConfig, 
...
var myMessageProcessor = new msmp:MessageForwardingProcessor(myProcessorConfig, handleResponseFromBE);
...
```
